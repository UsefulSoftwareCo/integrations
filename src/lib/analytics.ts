 ...
 * data. Pageviews are captured on Astro page loads so client-side navigations
 * count without rerunning the inline head script.
 */
export const ANALYTICS_JS =
  ...
  `if(!/^(localhost$|127\\.|0\\.0\\.0\\.0$)/.test(location.hostname)){posthog.init("${POSTHOG_TOKEN}",{api_host:"/_i",ui_host:"https://us.posthog.com",defaults:"2026-05-30",capture_pageview:!1});document.addEventListener("astro:page-load",function(){posthog.capture("$pageview",{$current_url:location.href})})}`;
